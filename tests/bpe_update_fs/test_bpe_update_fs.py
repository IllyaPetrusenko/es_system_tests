...
from tests.Cassandra_session import execute_cql_from_orchestrator_operation_step_by_oper_id, \
    execute_cql_from_orchestrator_operation_step, get_date_execute_cql_from_orchestrator_operation_step_by_oper_id, \
    get_release_date
from tests.authorization import get_x_operation_id
from tests.bpe_update_fs.payloads import fs_update_full_own_money, fs_update_obligatory_own_money, \
    fs_update_full_treasury_money, fs_update_obligatory_treasury_money
...
from tests.presets import set_instance_for_request, update_fs
from useful_functions import prepared_cpid, get_period, get_new_period, get_access_token_for_platform_two, \
    get_human_date_in_utc_format, get_timestamp_from_human_date, is_it_uuid
...
class TestBpeCreateEI(object):
    ...
    #     payload = copy.deepcopy(fs_update_full_own_money)
    #     update_fs_response = bpe_update_fs(cpid, create_fs_response[2], payload, f"{create_fs_response[1]}")
    #     print(create_fs_response)
    #     print(update_fs_response)
    #     assert update_fs_response[0].text == "ok"
    #     assert update_fs_response[0].status_code == 202
    #     assert update_fs_response[1]["X-OPERATION-ID"] == update_fs_response[2]
    #     assert update_fs_response[1]["errors"][0]["code"] == "400.10.00.08"
