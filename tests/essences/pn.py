...
class PN:
    ...
    @allure.step('Cancel PN')
    def cancel_pn(self, cp_id, pn_id, pn_token):
        plan = requests.post(
            url=self.host_of_request + "/cancel/pn/" + cp_id + "/" + pn_id,
            headers={
                'Authorization': 'Bearer ' + self.access_token,
                'X-OPERATION-ID': self.x_operation_id,
                'Content-Type': 'application/json',
    ...
    # ---------------------------------------------------------------------------------------
    @allure.step('Insert Update PN: based on PN -  full, based on FS: own - full, based on EI: with items - full')
    def insert_pn_full_update(self, first_lot_id, second_lot_id, first_item_id, second_item_id):
        auth_provider = PlainTextAuthProvider(username=self.cassandra_username, password=self.cassandra_password)
        cluster = Cluster([self.cassandra_cluster], auth_provider=auth_provider)
        session = cluster.connect('ocds')
        owner = "445f6851-c908-407d-9b45-14b92f3e964b"
        ...
        json_orchestrator_context = {
            ...
            "stage": "PN",
            "prevStage": "PN",
            "processType": "updatePN",
            "operationType": "updatePN",
            "phase": "planning",
            "owner": owner,
            "country": self.country,
            "language": self.lang,
    ...
    # ==================================
    @allure.step('Insert Update PN: based on PN - obligatory,  based on FS: treasury - obligatory, '
                 'based on EI: without items - obligatory')
    def insert_pn_obligatory_update(self):
        auth_provider = PlainTextAuthProvider(username=self.cassandra_username, password=self.cassandra_password)
        cluster = Cluster([self.cassandra_cluster], auth_provider=auth_provider)
        session = cluster.connect('ocds')
        owner = "445f6851-c908-407d-9b45-14b92f3e964b"
