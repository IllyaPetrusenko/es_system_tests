...
class Enquiry:
    def __init__(self, instance, cassandra_username, cassandra_password, pmd, country='MD',
                 ...
                 document_two_id=None, document_three_id=None):
    ...
    @allure.step('Create Enquiry')
    def create_enquiry(self, cp_id, ev_id, payload):
        enquiry = requests.post(
            url=self.host_of_request + f"/do/enquiry/{cp_id}/{ev_id}",
            headers={
                'Authorization': 'Bearer ' + self.access_token,
                'X-OPERATION-ID': self.x_operation_id,
                'Content-Type': 'application/json'},
            json=payload)
        allure.attach(self.host_of_request + "/do/enquiry/", 'URL')
    ...
    def check_on_that_message_is_successfully_create_enquiry_without_token(self, cp_id, ev_id):
        instance_url = None
        ...
        check_enquiry_id = is_it_uuid(message["data"]["outcomes"]["enquiries"][0]["id"], 1)
        for i in message["data"]["outcomes"]["enquiries"][0].keys():
            if i == "X-TOKEN":
                raise Exception("enquiries_token must be missing")
        if check_x_operation_id is True and check_x_response_id is True and check_initiator is True and \
                check_oc_id is True and check_url is True and check_operation_date is True and check_enquiry_id is True:
            return True
        else:
            return False

    def check_on_that_message_is_successfully_create_enquiry_with_token(self, cp_id, ev_id):
        instance_url = None
        if self.instance == "dev":
            instance_url = "http://dev.public.eprocurement.systems/tenders/"
        if self.instance == "sandbox":
        ...
        check_x_operation_id = is_it_uuid(message["X-OPERATION-ID"], 4)
        check_x_response_id = is_it_uuid(message["X-RESPONSE-ID"], 1)
        check_initiator = fnmatch.fnmatch(message["initiator"], "bpe")
        check_oc_id = fnmatch.fnmatch(message["data"]["ocid"], ev_id)
        check_url = fnmatch.fnmatch(message["data"]["url"],
                                    f"{instance_url}{cp_id}/{ev_id}")
        check_operation_date = fnmatch.fnmatch(message["data"]["operationDate"], "202*-*-*T*:*:*Z")
        check_enquiry_id = is_it_uuid(message["data"]["outcomes"]["enquiries"][0]["id"], 4)
        check_enquiry_token = is_it_uuid(message["data"]["outcomes"]["enquiries"][0]["X-TOKEN"], 4)
        if check_x_operation_id is True and check_x_response_id is True and check_initiator is True and \
                check_oc_id is True and check_url is True and check_operation_date is True \
                and check_enquiry_id is True and check_enquiry_token is True:
            return True
        else:
            return False
    ...
    # =======================
    # @allure.step('Insert tender was suspended: based on CnOnPn: full with auction, based on PN: full,  '
    #              'based on FS: own - full, based on EI: with items - full')
    # def insert_suspended_tender_full(self, first_lot_id, second_lot_id, first_item_id, second_item_id,
    #                                  second_tender, second_enquiry):
    #     auth_provider = PlainTextAuthProvider(username=self.cassandra_username, password=self.cassandra_password)
    #     cluster = Cluster([self.cassandra_cluster], auth_provider=auth_provider)
    #     key_space_ocds = cluster.connect('ocds')
