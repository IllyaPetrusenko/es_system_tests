...
def insert_into_db_create_ei(cpid):
    ...
    timestamp = int(time.mktime(datetime.strptime(time_at_now_miliseconds, "%Y-%m-%dT%H:%M:%S.%fZ").timetuple())) * 1000
    ...
    return f"http://dev.public.eprocurement.systems/budgets/{cpid}/{cpid}", f"X-TOKEN == {ei_token}"
...
def insert_into_db_create_fs(cpid):
    ...
    return f"http://dev.public.eprocurement.systems/budgets/{cpid}/{fs_ocid}", f"X-TOKEN == {fs_token}"
